    ...
    : QWidget(parent), m_database(std::move(database))
{
    ...
    m_serialTable = new QTableWidget(0, 9, summaryPanel);
    m_serialTable->setHorizontalHeaderLabels({QStringLiteral("SN"), QStringLiteral("物料编码"),
                                              QStringLiteral("物料名称"), QStringLiteral("批次"),
                                              QStringLiteral("状态"), QStringLiteral("仓库"),
                                              QStringLiteral("库位"), QStringLiteral("入库时间"),
                                              QStringLiteral("最近单据")});
    ...
    m_historyTable = new QTableWidget(0, 8, historyPanel);
...
void SerialTracePage::loadSerials()
{
    ...
    query.prepare(QStringLiteral(
        "SELECT sn.id,sn.serial_no,m.code,m.name,sn.batch_no,sn.status,"
        "COALESCE(w.code,''),COALESCE(l.code,''),sn.inbound_at,COALESCE(d.document_no,'') "
...
void SerialTracePage::loadHistory()
{
    ...
    query.prepare(QStringLiteral(
        ...
        "l.quantity_before,l.quantity_after,w.code||' / '||loc.code "
        ...
        "JOIN locations loc ON loc.id=l.location_id WHERE x.serial_id=? ORDER BY l.id"));
    query.addBindValue(m_serialTable->item(row, 0)->data(SerialIdRole));
    query.exec();
    while (query.next()) {
